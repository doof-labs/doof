...
describe('human product pages', () => {
  ...
  it('serves the hosted-first home, shared styles and truthful promise', async () => {
    ...
    expect(html).toContain('<strong>+72–99</strong>');
    expect(html).toContain('<strong>19 / 36</strong>');
    expect(html).toContain('<strong>2,160</strong>');
    expect(html.indexOf('<strong>19 / 36</strong>')).toBeLessThan(html.indexOf('<strong>+72–99</strong>'));
    expect(html).toContain('On GPT models it beat a plain notify tool by 58 to 71 points.');
